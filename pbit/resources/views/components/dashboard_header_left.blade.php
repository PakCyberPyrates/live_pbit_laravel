<div class="dashboard_header_left">	
	<a href="{{url('/')}}"><img  src="{{ asset('public/frontview/images/logo.png') }}"  class="dashbord-logo dashboard-responsive" alt="logo"></a> 
</div>
...
<div class="dashboard_mid_left">
	<div class="user-profile">	
		<div class="inner-circle"></div>
			@php
				...
				if(file_exists('public/images/avatar/'.Auth::user()->avtar.'') && (Auth::user()->avtar !='')){ 
					...
					$image = asset('public/frontview/images/NoPicture.png');
				}				
			@endphp
		...
		<?php 
		 $userData =  App\Helper::get_user_byed_course(); 
		?>
		<ul class="navbar-nav dashboard_nav dashboard-space dashbord_dash_left">
			@if(count($userData)>0)	
			...
			<ul class="sub-menu collapse" id="mycourses">
				...
				<li class="nav-item li_tb tb-{{ $i }}">
					...
					@endphp
					<a href="{{url('/')}}/user/cource/{{$item_slug}}" class="nav-link">{{$course->course_name}} </a>
				</li>
				@php
				$i++;
				@endphp
			...
			@endphp
			@endif
		 	<li  class="nav-item {{$passClass}}"><a class="dropdown-item subdrop" href="{{url('myorder')}}"><i class="fas fa-list"></i> MyOrder</a></li>
			...
			<ul class="sub-menu collapse {{$class}}" id="products">
				...
				<li  class="nav-item {{$userClass}}"><a class="dropdown-item subdrop" href="{{url('user-profile')}}"><i class="fas fa-user"></i> Update Profile</a></li>
