			...
			<div class="card-body">
				<form class="form-horizontal form-bordered" id="edit-topic-form" action="{{ action('TopicController@update',[$topic->id])}}" method="post" enctype="multipart/form-data">
					...
					<div class="form-group row">
						...
						<div class="col-lg-6">
							<select  class="form-control populate" name="course_id"  id="course_id">
								...
								<?php $selected = ($course->id == $topic->course_id)?'selected=selected':'';?>
					...
					<div class="form-group row">
						...
						<div class="col-lg-6">
							<select  class="form-control populate {{ $errors->has('chapter_id') ? ' is-invalid' : '' }}" name="chapter_id" id="chapter_id">
								...
								<ul class="thumd-list">
									@if(count($topic->video))
										...
										if(!empty($video->thumb_url)){
										  	$thumb = url('public/uploads/videos/thumb').'/'.$video->thumb_url;
										}
										$video_link = App\Helper::get_link($video->url,'topic'); // get a link that expire in 1 hour
										?>
						...
						<div class="col-lg-5">
							<ul class="thumd-list">
								@if(count($topic->trailer_video))
									...
									$thumb = url('public/img/videos-default.jpg');
									if(!empty($trail_video->trailer_thumb_url)){
									  	$thumb = url('public/uploads/videos/thumb').'/'.$trail_video->trailer_thumb_ur;
									}

									$video_link = App\Helper::get_link($trail_video->trailer_url,'topic'); // get a link that expire in 1 hour
									?>
									<li> 
...
$(document).ready(function(){
  ...
  active_nav_links(6,active_link);
  	...
  	$('#edit-topic-form').bootstrapValidator({
        ...
        fields: {
            ...
            price: {
                ...
                validators: {
                    ...
                    regexp: {
                        regexp: /^[0-9]+$/,
                        message: 'The price can only consist of alphabetical, dot and underscore'
                    }
                }
            }
        }
