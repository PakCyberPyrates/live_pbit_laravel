            ...
            <section class="card">
              ...
              <div class="card-body">
                ...
                <table class="table table-bordered table-striped mb-0" id="datatable-default">
                  ...
                  <tbody> 
                    <?php $style = 'un-view-order'; ?>
                      @foreach($orders as $key => $order)
                      <tr  class="{{(!$order->read_status)?$style:''}}" >
                      <th scope="row">#{{$order->id}}</th>
                      <td>{{$order->user->email}}</td>
                      <td>{{count($order->items)}}</td>
                      <td>${{ number_format($order->amount,2,'.','') }}</td>
                      <td><label class="label label-{{$order->payment_getway == 'stripe'?'primary':'warning'}}">{{$order->payment_getway}}</label></td>
                      ...
                      <td>{{date('d-m-Y h:s a',strtotime($order->created_at))}}</td>
                      <td> 
