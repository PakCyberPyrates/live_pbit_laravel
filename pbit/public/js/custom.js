                        ...
                        $.ajax({
                            ...
                            if(response.orders_lists.length >0){
	                            $.each(response.orders_lists,function(inx,val){
	                            	if (val.userimage){
	                                     avatar = BASE_URL+'/public/images/avatar/'+val.userimage;
	                                }
	                                html += '<li><a href="'+BASE_URL+'/admin/orders/'+val.id+'" class="clearfix"><figure class="image"><img src="'+avatar+'" alt="Image" class="rounded-circle" /></figure><span class="title">Order #'+val.id+'<span class="message">'+val.uname+'</span><span class="message">'+timing(new Date(val.created_at))+'</span></a></li>';
