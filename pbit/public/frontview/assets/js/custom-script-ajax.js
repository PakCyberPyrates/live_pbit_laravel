...
jQuery(document).ready(function(){         /*Login-Section*/
      ...
      if (jQuery.trim(F_Email).length == 0 ) {
        ...
        //alert('Email is required');
         ...
         if(F_emailstatus==true){
            ...
            data: {
               _token: jQuery('input[name="_token"]').attr('value'),
              email: 'test.shinedezign@gmail.com',                                  
            },
            success: function(result){
...
jQuery('.signup').click(function(e){                  
  ...
  jQuery("#DashboardSearch").keyup(function(event){
    ...
    if(event.keyCode == 32){
      ...
      var loaderurl=BASE_URL+"/public/frontview/images/ajax-loader.gif";
      $("#responseData").html("<li><img src="+loaderurl+"></li>");
      ...
      jQuery.ajax({
        ...
        success: function(result){
          ...
          if(result != ''){
            ...
            if(!this['chapter_id']){
                  var slugurlchapter=slug(this['title']);
                  var course_name=slug(this['course_name']);                                    
                  url = BASE_URL +'/user/cource/'+course_name+'/'+slugurlchapter;
                  var title = this['title'].charAt(0).toUpperCase() + this['title'].slice(1);
              }
              else{
               /* user/cource/beginner/what-is-lorem-ipsum-beginner/*/
                var slugurltopic=slug(this['title']);
                var course_name=slug(this['course_name']);  
                var chapter_slug=slug(this['course_name']);  
                var chapter_name=slug(this['chapter_name']);  
                url = BASE_URL +'/user/cource/'+course_name+'/'+chapter_name+'/'+slugurltopic;
                var title = this['title'].charAt(0).toUpperCase() + this['title'].slice(1);
  ...
  $("#userSearch").keyup(function(event){
   ...
   var loaderurl=BASE_URL+"/public/frontview/images/ajax-loader.gif";
  $("#searchul").html("<li><img src="+loaderurl+"></li>");
          ...
          var topicid = '';
              ...
              if(!this['course_id']){
                  url = BASE_URL +"/course/"+this['title'];
                      ...
                      var slugurl=slug(this['title']);                 

                  url = BASE_URL + "/topic/"+slugurl;
              }
              var title = this['title'].charAt(0).toUpperCase() + this['title'].slice(1);
...
$(document).ready(function(){
	...
	$("#hsearch").keyup(function(event){
      ...
      var loaderurl=BASE_URL+"/public/frontview/images/ajax-loader.gif";
      $("#searchultop").html("<li><img src="+loaderurl+"></li>");
			...
			success: function(result){
                ...
                var slugurl=slug(this['title']);
							...
							var title = this['title'].charAt(0).toUpperCase() + this['title'].slice(1);
							if(this['title'] == 'Record not Found'){
								$("#searchultop").append("<li><a style='color:#000' href='javascript:void(0);'>" + title + "</a></li>");              
							}else{
